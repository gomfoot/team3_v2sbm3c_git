...
<script type="text/javascript">
  function delete_func(favoritesno) {  // GET -> POST 전송, 상품 삭제
    var frm = $('#frm_post');
    $('#favoritesno',  frm).val(favoritesno);
...
<form name='frm_post' id='frm_post' action='./delete.do' method='post'>
  <input type='hidden' name='favoritesno' id='favoritesno'>
</form>
...
<DIV class='content_body'>
  ...
  <div style='width: 100%;'> <%-- 갤러리 Layout 시작 --%>
    <c:forEach var="favoritesVO" items="${list }" varStatus="status">
            <c:set var="favoritesno" value="${favoritesVO.favoritesno }" />
            <c:set var="movieno" value="${favoritesVO.movieno }" />
            <c:set var="mtitle" value="${favoritesVO.mtitle }" />
            <c:set var="imgthumb1" value="${favoritesVO.imgthumb1 }" />
            <c:set var="imgsize1" value="${favoritesVO.imgsize1 }" />
            <c:set var="memberno" value="${favoritesVO.memberno }" />
            ...
            <c:choose> 
              <c:when test="${imgthumb1.endsWith('jpg') || imgthumb1.endsWith('png') || imgthumb1.endsWith('gif')}"> <!-- 이미지 인경우 -->
                ...
                <td style='vertical-align: middle; text-align: center;'>               
                  <A href="javascript: delete_func(${favoritesno })"><IMG src="/favorites/images/delete3.png"></A>
                </td>
              </c:when>
