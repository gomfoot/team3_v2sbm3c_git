...
public class FavoritesProc implements FavoritesProcInter {
  ...
  public FavoritesProc(){
    System.out.println("-> FavoritesProc created.");
  }

  @Override
  public int create(FavoritesVO favoritesVO) {
    int cnt = this.favoritesDAO.create(favoritesVO);
    return cnt;
  }
  
  @Override
  public int search_count(HashMap<String, Object> hashMap) {
    int count = favoritesDAO.search_count(hashMap);
    return count;
  }
...
//    str.append("현재 페이지: " + nowPage + " / " + totalPage + "  "); 
  ...
  public List<FavoritesVO> list_favorites(HashMap<String, Object> map) {
    ...
    */
    ...
    List<FavoritesVO> list = this.favoritesDAO.list_favorites(map);
    
    for (FavoritesVO favoritesVO : list) { // 내용이 160자 이상이면 160자만 선택
      
      String mtitle = Tool.convertChar(favoritesVO.getMtitle());
      favoritesVO.setMtitle(mtitle);
  ...
  public int delete(int favoritesno) {
    int cnt = this.favoritesDAO.delete(favoritesno);
    return cnt;
  }
  
  @Override
  public int favorites_check(HashMap<String, Object> hashMap) {
    int cnt = favoritesDAO.favorites_check(hashMap);
    return cnt;
  }
}
