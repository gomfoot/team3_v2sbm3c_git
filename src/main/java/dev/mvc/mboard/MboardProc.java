...
import dev.mvc.notice.Contents;
import dev.mvc.notice.NoticeVO;
import dev.mvc.tool.Tool;

@Component("dev.mvc.mboard.MboardProc")
public class MboardProc implements MboardProcInter {
  @Autowired  // DI: Spring 자동으로 구현한 DAO class 객체를 할당
  private MboardDAOInter mboardDAO;

  @Override
  public int create(MboardVO mboardVO) {
    int cnt = mboardDAO.create(mboardVO);
    
    return cnt;
  }
  @Override
  public int search_count() {
    int count = mboardDAO.search_count();
    return count;
  }

  @Override
  public MboardVO read(int mboardno) {
    MboardVO mboardVO =this.mboardDAO.read(mboardno);
    return mboardVO;
  }
...
List<MboardVO> list = this.mboardDAO.list_all(map);
    
    for (MboardVO MboardVO : list) { // 내용이 160자 이상이면 160자만 선택
      String title = MboardVO.getTitle();
      if (title.length() > 160) {
        title = title.substring(0, 160) + "...";
        MboardVO.setTitle(title);
      }
    }
...
//    str.append("현재 페이지: " + nowPage + " / " + totalPage + "  "); 
  ...
  public int delete(int mboardno) {
   int cnt = this.mboardDAO.delete(mboardno);
   return cnt;
  }
  ...
  public int update(MboardVO mboardVO) {
    int cnt = this.mboardDAO.update(mboardVO);
    return cnt;
  }
  @Override
  public int cnt_update(int mboardno) {
    int cnt=this.mboardDAO.cnt_update(mboardno);
    return cnt;
  }
